$(function(){
  ...
  const globalState = {
    apps: [
      {
        name: 'Calendar',
        icon: '/simulator/Icons/Calendar.png',
        type: 'widgetFull',
        dynamic: true
      },
      {
        name: 'Weather',
        icon: '/simulator/Icons/Weather.png',
        type: 'app',
        dynamic: false
      },
      {
      ...
      {
        name: 'Photos',
        icon: '/simulator/Icons/Photos.png',
        type: 'app',
        dynamic: false
      },
      {
        name: 'Maps',
        icon: '/simulator/Icons/GoogleMaps.png',
        type: 'app',
        dynamic: false
      },
      {
        name: 'Camera',
        icon: '/simulator/Icons/Camera.png',
        type: 'app',
        dynamic: false
      },
      {
      ...
      {
        name: 'Calculator',
        icon: '/simulator/Icons/Calculator.png',
        type: 'app',
        dynamic: false
      },
      {
...
<iframe src="/simulator/apps/Camera.html" style="width="290px" height="410px" scrolling="no""></iframe>
</div>
...
<path d="M62 38l-7.259-9.86L46 36"></path>
  ...
  function music() {
    if (!$('.musicApp').length) {
      ...
      <div class="musicApp hidden">
	     <div class="appScreen">
            <div class="contents">
	            <iframe src="/simulator/apps/music-app/index.html" style="width="280px" height="540px""></iframe>
			</div>
		 </div>
      </div>
      `
  ...
  function maps() {
    ...
    if (!$('.mapsApp').length) {
      $('.mainScreen').append(`
        <div class="mapsApp hidden">
          <div class="appScreen" style="overflow:hidden;">
            <div id="iphoneLeafletMap" style="width:100%;height:100%;"></div>
          </div>
        </div>`
      );
      $('.mapsApp').touchMov({
      ...
      // Init Leaflet map
      setTimeout(function () {
        const el = document.getElementById('iphoneLeafletMap');
        if (el && !el._leaflet_map) {
          const map = L.map(el, { zoomControl: false, attributionControl: false, center: [20, 0], zoom: 2 });
          el._leaflet_map = map;
          el._markerLayer = L.layerGroup().addTo(map);
          // Use same tile layer preference as HUD
          var tileKey = 'street';
          if (s) { var pref = s.get('map.tileLayer'); if (pref) tileKey = pref; }
          var tileUrl = 'https://tile.openstreetmap.org/{z}/{x}/{y}.png';
          if (tileKey === 'dark') tileUrl = 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png';
          else if (tileKey === 'satellite') tileUrl = 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}';
    ...
    setTimeout(function(){
      var el = document.getElementById('iphoneLeafletMap');
      if (el && el._leaflet_map) {
  ...
  function refreshIphoneMap() {
    ...
    // 1. User marker
    var coords = s.get('user.currentCoords');
    if (coords && coords.lat) {
      map.setView([coords.lat, coords.lon], 15);
      L.marker([coords.lat, coords.lon], {
        icon: L.icon({ iconUrl: '/spidey-marker.png', iconSize: [32, 32], iconAnchor: [16, 16] }),
        zIndexOffset: 1000,
      }).addTo(layer);
    }

    // 2. Mission markers
    var current = s.get('missions.current');
    var pending = s.get('missions.pending') || [];
    var allM = current ? [current].concat(pending) : pending.slice();
    allM.forEach(function(m) {
      var loc = m.locationCoords;
      if (loc && loc.lat && loc.lon) {
        L.marker([loc.lat, loc.lon], {
          icon: L.icon({ iconUrl: '/mission-marker.png', iconSize: [28, 28], iconAnchor: [14, 14] }),
        }).addTo(layer).bindPopup('<b>' + m.title + '</b>');
      }
    });

    // 3. Quest markers
    var quests = s.get('sideQuests') || [];
    quests.forEach(function(q) {
      if (q.completed || !q.locationCoords) return;
      var qc = q.locationCoords;
      if (qc && qc.lat && qc.lon) {
        L.marker([qc.lat, qc.lon], {
          icon: L.icon({ iconUrl: '/sidequest-marker.png', iconSize: [24, 24], iconAnchor: [12, 12] }),
        }).addTo(layer).bindPopup('<b>' + q.title + '</b>');
      }
    });
  }
    ...
    if (!$('.calculatorApp').length) {
      $('.mainScreen').append(`
        <div class="calculatorApp hidden">
          <iframe src="/simulator/apps/Calculator.html" style="width:100%;height:100%;border:none;display:block"></iframe>
        </div>`
      );
    ...
    if (!$('.weatherApp').length) {
      $('.mainScreen').append(`
        <div class="weatherApp hidden">
	     <div class="appScreen">
            <div class="contents">
               <iframe src="/simulator/apps/weather-app-final/index.html" style="width="400px" height="600px" background-color="white""></iframe>
			</div>
		 </div>
        </div>`
      );
    ...
    if (!$('.contactsApp').length) {
      $('.mainScreen').append(`
        <div class="contactsApp hidden">
	     <div class="appScreen">
            <div class="contents">
	            <iframe src="/simulator/apps/Contacts/Contacts.html" style="width="300px" height="600px""></iframe>
			</div>
		 </div>
        </div>`
      );
  ...
  function photos() {
    if (!$('.photosApp').length) {
      $('.mainScreen').append(`
        <div class="photosApp hidden">
          <iframe src="/simulator/apps/Photos/photos.html" style="width:100%;height:100%;border:none;"></iframe>
        </div>`
      );
      $('.photosApp').touchMov({
        mov: 'y',
  ...
  function safari() {
    alertiOS({
      headers: 'Safari',
      messages: 'Safari is not available in this demo.',
      actions: [{ text: 'OK', warning: false }]
    });
  }
  ...
  // widgetCenter swipe up → go home
  $('.widgetCenter').touchMov({
    mov: 'y',
    movUp: function (e) {
      $(e.currentTarget).addClass('hidden');
      $(e.currentTarget).parents('.mainScreen').removeClass('blur');
      $(e.currentTarget).siblings('.appScreen').removeClass('moveOut');
...
`);
  ...
  $('.touchID').click(function () {
    if (!$(this).hasClass('active')) {
      let sound = new Audio('/simulator/assets/Sound.mp3');
      sound.play();
    }
    $('#iOSAlert').remove();
    $(this).toggleClass('active');
  ...
  // Wallpaper from Photos app (iframed)
  const savedWall = localStorage.getItem('spidey_wallpaper');
  if (savedWall) {
    document.querySelector('.mainScreen').style.backgroundImage = `url("${savedWall}")`;
  }
  window.addEventListener('message', (e) => {
    if (e.data && e.data.type === 'setWallpaper') {
      document.querySelector('.mainScreen').style.backgroundImage = `url("${e.data.data}")`;
      localStorage.setItem('spidey_wallpaper', e.data.data);
    }
  });
})
